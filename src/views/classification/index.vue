      ...
      <div class="leftWrapper">
        <ul class="wrapper">
          <li class="categoryItem"
              v-for="(spu, index) in spuData"
              :class="{ selected: currentIndex === index }"
              @click="clickLeftLi(spu.id, index)"
              :key="spu.id"
              ref="menuList">
            <span class="textWrapper">{{ spu.name }}</span>
          </li>
        </ul>
      </div>
      <!--右边-->
      <ContentView :classificationData="classificationData" />
    </div>
    <!-- 骨架屏 -->
...
import { postSpus, postClassification } from '@/api/home.js'
// 6.引入骨架屏
...
export default {
  ...
  methods: {
    ...
    async _initData () {
      // 1.1 获取左边的数据
      await postSpus().then(response => {
        this.spuData = response.data

        console.log(response.data[0].id)
        postClassification({ spuId: response.data[0].id }).then(response => {
    ...
    clickLeftLi (spuId, index) {
      ...
      // 2.4 获取右边的数据
      postClassification({ spuId: spuId }).then(response => {
        this.classificationData = response.data
